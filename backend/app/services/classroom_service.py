...
def _generate_visualization3d_html_direct(
    ...
    *,
    ...
    user_prompt = (
        '生成一个完整的交互式 3D 演示 HTML 页面。直接输出 HTML，不要任何解释。\n\n'
        '## Three.js 版本与可用 API\n'
        '- 使用 Three.js r160 (经过验证的稳定版本):\n'
        '  importmap: { “three”: “https://unpkg.com/three@0.160.0/build/three.module.js”,\n'
        '              “three/addons/”: “https://unpkg.com/three@0.160.0/examples/jsm/” }\n'
        '- import { OrbitControls } from “three/addons/controls/OrbitControls.js”;\n'
        '- 可用的几何体: SphereGeometry, BoxGeometry, CylinderGeometry, TorusGeometry, PlaneGeometry,\n'
        '  RingGeometry, BufferGeometry, TubeGeometry\n'
        '- 可用的材质: MeshStandardMaterial, MeshPhongMaterial, MeshBasicMaterial, LineBasicMaterial,\n'
        '  LineDashedMaterial, PointsMaterial, SpriteMaterial\n'
        '- 可用的光源: AmbientLight, DirectionalLight, HemisphereLight, PointLight, SpotLight\n'
        '- 可用辅助: ArrowHelper, AxesHelper, GridHelper, PolarGridHelper\n'
        '- THREE.Raycaster 用于点击检测\n\n'
        '## 严禁使用的 API (这些在 r160 中不存在或已废弃，用了就会报错)\n'
        '- 禁止 THREE.Font / new THREE.Font() -- 用 HTML div + CSS 定位做标签\n'
        '- 禁止 THREE.TextGeometry / THREE.TextBufferGeometry\n'
        '- 禁止 THREE.CSS2DRenderer / THREE.CSS3DRenderer\n'
        '- 禁止 FontLoader / TTFLoader\n'
        '- 禁止 EffectComposer / UnrealBloomPass (需要额外 addon 路径)\n\n'
        '## 技术要求\n'
        '1. 静态 import (不要动态 import())，importmap 加载\n'
        '2. 光照: AmbientLight(0x404040,2) + DirectionalLight(0xffffff,1.5) + HemisphereLight(0x80a0ff,0x201030,1)\n'
        '3. 响应式，铺满窗口，深色背景 #0b1120\n'
        '4. 底部控制栏: 播放/暂停/重置按钮 + 速度滑块 + 步骤跳转按钮\n'
        '5. 左上角信息面板: 标题和当前步骤说明\n'
        '6. 标签用 HTML div + CSS absolute 定位，每帧 project() 后更新 transform\n\n'
        f'## 场景内容\n'
        f'- 项目: {project.title}\n'
        f'- 学习项: {item.title}\n'
        f'- 知识点: {item.knowledge_points}\n'
        ...
        f'- 课堂摘要: {package.learning_summary}\n\n'
        '## 场景设计\n'
        '- 4-8 个 3D 对象 (球体/立方体/圆柱)，颜色区分角色\n'
        '- 对象间用 Line 或 TubeGeometry 连接\n'
        '- HTML div 标签通过 vector.project(camera) 计算屏幕坐标定位\n'
        '- 4-6 个步骤帧，每帧切换高亮对象和说明文字\n'
        '- 用 Points + BufferGeometry 做粒子效果\n\n'
        '## 必须遵守的代码模板\n'
        '```javascript\n'
        ...
        '- try-catch 包裹初始化，catch 中显示 err.message 到 #error-message\n'
        '- 加载时显示 loading 遮罩\n'
    )
